...
class PasswordGeneratorApp:
    ...
    def build_ui(self):
        ...
        ttk.Entry(result, textvariable=self.password_var,
    ...
    def generate_password(self):
        ...
        if length < 8:
            ...
            return
        if len(selected) < 2:
            messagebox.showwarning("Character Types Required","Please select at least 2 character types.")
            return
        selected = [self.clean_charset(s) for s in selected]
        if any(not s for s in selected):
            messagebox.showwarning("Invalid Selection","A selected character type has no usable characters.")
            return
